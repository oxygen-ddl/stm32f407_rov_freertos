...
void view_variables_Task(void *argument)
{
    for (;;)
    {
        ...
        //UART_SendFloats_DMA(8,(float)light.on,(float)mode.lockangle,(float)mode.unknow,(float)mode.autotrip,(float)mode.electromagnet,(float)mode.push_rod,(float)mode.autovertical,(float)mode.defogging);
